...
class ShortenURLModel extends CI_Model {
    ...
    public function getShortURL(&$dbc, &$url, &$url_md5) {
        ...
        if(!is_null($check)) { //URL exists in database
            ...
            $url_time = time(); //Assign a new timestamp. The timestamp is actually the short form of the URL as the time is always increasing therefore, it is always unique.
