...
class ShortenURLModel extends CI_Model {
    ...
    public function getFullURL(&$dbc, &$url_time) {
        ...
        $statement = new Cassandra\SimpleStatement("SELECT url_md5 FROM url_front WHERE url_time = ? ;");
        $options = new Cassandra\ExecutionOptions(["arguments" => [
            new Cassandra\Bigint($url_time)
        ]]);
        ...
        if(!is_null($check)) {
            ...
            if(!is_null($check)) {
                ...
                unset($statement);
                unset($options);
                $statement = new Cassandra\SimpleStatement("SELECT url_full FROM url WHERE url_md5 = ? ;");
                $options = new Cassandra\ExecutionOptions(["arguments" => [$url_md5]]);
                $data = $dbc->execute($statement, $options);
                if(!is_null($data)) {
                    $data = $data->first();
